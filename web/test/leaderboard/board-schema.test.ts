...
test("boardQuerySchema defaults offset to 0 and coerces query strings", () => {
  assert.equal(boardQuerySchema.parse({}).offset, 0);
  assert.equal(boardQuerySchema.parse({ offset: "50" }).offset, 50);
});
